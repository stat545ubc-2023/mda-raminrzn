...
#PART1: Here I'm going to compare the range, mean and maximum and minimum number of building storeys among buildings with and without air conditioning.
...
#PART1: Computing the proportion and counts of having a balcony across the groups of being non-smoking or not.
...
#PART2: This graph visualizes the count of properties with and without balconies for both non-smoking and smoking groups. There are two geom layers: The first  creates the bars representing the counts. The second layer adds text labels to the top of the bars to display the count for each category.
...
# Convert the specified variables to factor using dplyr functions
apt_buildings <- apt_buildings %>%
...
# View the data frame with the new Amenity_Score variable
head(apt_buildings)
...
# Calculate summary statistics for number of units based on number of amenity categories
summary_data <- apt_buildings %>%
...
Based on my current findings, I am closer to answering all four of my research questions,Considering that I have observed correlations for all the initial hypotheses I had. Although, more refinements can be made on each question, the current setup seems to have been adeqaute in getting answers. Questions three and four are the most interesting. As graphs from Q3 show that the newer builds of apartments do infact have more amenities, proving we are continuously seeking more comfort and Q4 shows, as the number of occupants is increased in each building, we see a trend of increased amenities, making it more accessible to everyone.
...
id: This column represents seemingly unique identification numbers for each of the apartments.
...
apt_buildings_ut <- pivot_longer(apt_buildings, 
                       cols = air_conditioning,  # Specify the column to pivot
                       names_to = "air_conditioning_row",  # New row variable name
                       values_to = "air_conditioning_value",  # New value variable name
...
# The resulting data frame (original state)
...
The following summarizes the regression coefficients and associated statistics for the modelin a tibble using broom. The results indicate that as "Building_Age" increases, "Amenity_Score" tends to decrease. The p-value for "Building_Age" is also very close to zero, indicating that it is highly statistically significant. 
